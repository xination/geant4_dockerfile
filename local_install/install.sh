...
echo "-----build geant4-----" && \
cd $HOME && \
sudo mkdir -p $HOME/geant4/src && \
sudo mkdir -p $HOME/geant4/build && \
sudo mkdir -p $HOME/geant4/install && \
sudo mkdir -p $HOME/geant4/data && \
sudo wget http://cern.ch/geant4-data/releases/geant4.${G4_VERSION}.tar.gz && \
sudo tar xf geant4.${G4_VERSION}.tar.gz -C $HOME/geant4/src && \
...
cmake -DCMAKE_INSTALL_PREFIX=$HOME/geant4/install \
      ...
      -DGEANT4_INSTALL_DATADIR=$HOME/geant4/data \
